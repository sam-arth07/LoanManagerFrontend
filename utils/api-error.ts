...
export async function handleApiResponse<T>(
  ...
  if (!response.ok) {
    ...
    try {
      ...
      errorMessage = errorData.error || errorData.message || customErrorMessage;
    } catch (e) {
      // If parsing fails, use status text or default message
      errorMessage = response.statusText || `${customErrorMessage} (${response.status})`;
    }
...
export async function withErrorHandling<T>(
  ...
  const attemptCall = async (): Promise<{ data: T | null; error: string | null }> => {
    try {
      ...
      if (opts.retry && retryAttempt === 0) {
        ...
        // Log retry attempt if not silent
        if (!opts.silent) {
          console.log(`Retrying API call after error: ${err instanceof Error ? err.message : 'Unknown error'}`);
        }
      ...
      // Log error if not in silent mode
      if (!opts.silent) {
        console.error('API Error:', err);
        
        // Add additional debug info for ApiErrors
        if (err instanceof ApiError) {
          console.debug(`Status code: ${err.statusCode}, Message: ${err.message}`);
        }
      }
